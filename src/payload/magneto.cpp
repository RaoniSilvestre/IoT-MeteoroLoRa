...
esp_err_t magneto_init() {
    if (sensor_write_reg(QMC5883L_ADDRESS, QMC5883L_REG_SET_RESET_PERIOD, 0x01) != ESP_OK) {
...
esp_err_t magneto_set_mode(uint8_t mode, uint8_t odr, uint8_t rng, uint8_t osr) {
    ...
    return sensor_write_reg(QMC5883L_ADDRESS, QMC5883L_REG_CTRL_1, ctrl1);
}

esp_err_t magneto_soft_reset() {
    Serial.println("[Magneto] Resetando sensor");
    return sensor_write_reg(QMC5883L_ADDRESS, QMC5883L_REG_CTRL_2, QMC5883L_CTRL_2_SFT_RST);
}

esp_err_t magneto_read(magneto_data_t *data) {
    uint8_t status;
    if (sensor_read_reg(QMC5883L_ADDRESS, QMC5883L_REG_STATUS, &status) != ESP_OK) {
    ...
    uint8_t buffer[6];
    if (sensor_read_reg(QMC5883L_ADDRESS, QMC5883L_REG_X_LSB, buffer, 6) != ESP_OK) {
...
esp_err_t magneto_read_temp(int16_t *temp) {
    uint8_t buffer[2];
    if (sensor_read_reg(QMC5883L_ADDRESS, QMC5883L_REG_TEMP_LSB, buffer, 2) != ESP_OK) {
